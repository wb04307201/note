...
grep -rl "RAG" 09.ai-applications/ | head -10
...
- ❌ 不要再写 `note/` 前缀路径 —— 本仓库内容已平铺到根,路径一律相对仓库根(如 `09.ai-applications/rag/`)
...
**历史教训(原仓库 Session 6)**:230 处断链 = 87% 来自"结构重组遗留路径错位"。**唯一可靠标准是 `os.path.isfile(target_abs)`**,不要相信"我觉得路径是对的"。
